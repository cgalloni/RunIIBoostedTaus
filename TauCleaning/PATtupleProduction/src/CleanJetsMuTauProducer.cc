...
CleanJetsMuTauProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  int useMuonId =1; //1=Muon soft; 2=Loose PF; 3= highPtMuon; 4=Tight PF
  ...
  for(reco::PFJetCollection::const_iterator iJet = PFJets->begin(); iJet != PFJets->end(); ++iJet){
	...
	switch( useMuonId )
	  {
	  case (1) : 
	    if  (muon::isGoodMuon(*theRecoMuon ,muon::TMOneStationTight)==true // SoftMoun
		 && theRecoMuon->innerTrack()->hitPattern().trackerLayersWithMeasurement() > 5
		 && theRecoMuon->innerTrack()->hitPattern().pixelLayersWithMeasurement() > 0
		 && theRecoMuon->innerTrack()->quality(reco::TrackBase::highPurity)==true
		 && fabs(theRecoMuon->innerTrack()->dxy(primaryVertex.position())) < 0.3 
		 && fabs(theRecoMuon->innerTrack()->dz(primaryVertex.position())) < 20.) selected = true;
	    break;
	  case (2) :	
	    if ((theRecoMuon->isGlobalMuon() ||theRecoMuon->isTrackerMuon()) && theRecoMuon->isPFMuon() )selected = true;
	     break;
	  case (3) :	
	    if (muon::isHighPtMuon(*theRecoMuon, primaryVertex//, muon::TunePType=muon::improvedTuneP
				   ) )selected = true;
	    break;
	  case (4) :
	    if (theRecoMuon->isGlobalMuon() && theRecoMuon->isPFMuon()
		&& theRecoMuon->globalTrack()->normalizedChi2() < 10
		&& theRecoMuon->numberOfMatchedStations() > 1
		&& fabs(theRecoMuon->muonBestTrack()->dxy(primaryVertex.position()))< 0.2
		&& fabs(theRecoMuon->muonBestTrack()->dz(primaryVertex.position())) < 0.5
		&& theRecoMuon->innerTrack()->hitPattern().numberOfValidPixelHits() > 0
		&& theRecoMuon->innerTrack()->hitPattern().trackerLayersWithMeasurement() > 5  ) selected =true;
	    break;
	  }
